...
public class AutoScrollViewPager extends ViewPager {
    ...
    /** 自动滑动时切换时间scroll factor for auto scroll animation, default is 1.0 **/
    private double                 autoScrollFactor            = 1.0;
    /** 手指滑动时切换时间scroll factor for swipe scroll animation, default is 1.0 **/
    ...
    private void init() {
        handler = new MyHandler(this.getWeakReference());
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        //下面注释代码作用用于ViewPager嵌套子ViewPager解决冲突
	...
	private  class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
          ...
          //检查消息队列并移除未发送的消息，这主要是避免在复杂环境下消息出现重复等问题。
            if(handler.hasMessages(SCROLL_WHAT)){
            	handler.removeMessages(SCROLL_WHAT);
            }
            switch (msg.what) {
                case SCROLL_WHAT:

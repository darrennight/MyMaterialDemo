 ...
 */
public class PlanPieView extends View {
    ...
    private Runnable threadAnimator = new Runnable() {
        @Override public void run() {
            boolean needNewFrame = false;
            for (PieHelper pie : pieHelperList) {
                pie.update();
    ...
    public PlanPieView(Context context) {
        this(context, null);
    }

    public PlanPieView(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public PlanPieView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {

...
public class MyPieChart extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < pieEntries.size(); i++) {
            ...
            float tempStart = startC;
            float tempSweep = sweep;
            //判断当前扇形是否被选中，确定用哪个半径
            float radiusT;
            if (pieEntries.get(i).isSelected()) {
                radiusT = sRadius;
                tempStart += mGrownPieGap;
                tempSweep -= 2 * mGrownPieGap;
            } else {
                radiusT = radius;
            }
            //画扇形的方法
            RectF rectF = new RectF(centerX - radiusT, centerY - radiusT, centerX + radiusT, centerY + radiusT);
            canvas.drawArc(rectF, tempStart, tempSweep, true, paint);
